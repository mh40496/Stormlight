...
/* TODO: 
			* An infusion expiry function. After 1 week has elapsed, the stormlight would be drained from the sphere
			* Variable to keep track of whether the sphere is authentic
*/

public class Sphere {
	/* Variables */

	private char gem; // Gem type
	private char size; // Gem size
	private char infused; // Whether the gem is infused with Stormlight
	...
	public Sphere( char inGem, char inSize ) {
		...
		setSize( inSize );
		infused = 'F';
	} // END of Sphere( Character, Character )
	
	// ALTERNATE
	public Sphere( char inGem, char inSize, char inFuse ) {
		setGem( inGem );
		setSize( inSize );
		setInfused( inFuse );
	} // END of Sphere( Character, Character, Boolean )
	...
	public void setInfused( char inFuse ) {
		inFuse = Character.toUpperCase( inFuse );
		
		if( !( validInfused( inFuse ) ) )
			inFuse = 'F';
	...
	// Method to check if parameter is a valid gem type Character
	// PARAMETER: Character inGem
	// OUTPUT: Boolean
	public boolean validGem( char inGem ) {
		return ( inGem == 'D' || inGem == 'G' || inGem == 'R' || inGem == 'S' || inGem == 'E');
	} // END of validGem
	
	// Method to check if parameter is a valid gem size Character
	// PARAMETER: Character inSize
	// OUTPUT: Boolean
	...
	public boolean validInfused( char inFused ) {
		return ( inFused == 'T' || inFused == 'F' );
	}
	
	// Method to set variable 'infused' to true
	// PARAMETER: -
	// OUTPUT: -
	public void infuse() {
		infused = 'T';
	...
	// Method to set variable 'infused' to false
	// PARAMETER: -
	// OUTPUT: -
	public void breath() {
		infused = 'F';
